...
check('storm-legacy-cone-suppressed', files.runtime.includes('ProductionMiddleVortex') && files.runtime.includes('ProductionDarkCore') && files.runtime.includes('funnelMat.opacity = 0.035') && files.guard.includes('funnelMat.opacity = 0.035') && files.runtime.includes("SWVisualSlice4VolumeShell')") && files.guard.includes("SWVisualSlice4VolumeShell')"));
check('storm-box-debris-suppressed', files.runtime.includes("object.isInstancedMesh && object.geometry?.type === 'BoxGeometry'") && files.guard.includes("object.isInstancedMesh && object.geometry?.type === 'BoxGeometry'") && files.qa.includes('visibleProductionTornadoBoxDebrisCount'));
check('storm-square-point-effects-suppressed', files.runtime.includes("typeof particleSystem !== 'undefined'") && files.guard.includes("typeof particleSystem !== 'undefined'") && files.qa.includes('visibleLegacyStormPointCount'));
check('storm-uses-no-pixel-atlas-cards', !files.runtime.includes('map: smoke') && !files.runtime.includes('map: dirt'));
check('storm-ground-rings-suppressed', files.runtime.includes("geometryType !== 'RingGeometry'") && files.guard.includes("geometryType !== 'RingGeometry'") && files.guard.includes("geometryType !== 'TorusGeometry'") && files.guard.includes('object.visible = false') && files.guard.includes('visibleLegacyStormRingCount'));
check('storm-ragged-wedge-profile', files.runtime.includes("silhouetteProfile: 'ragged-wedge-storm-v4-mesh-ribbons'") && files.runtime.includes('ragged condensation-column/wedge'));
...
check('storm-has-no-slice6-billboard-primitives', !files.runtime.includes('new THREE.Sprite') && !files.runtime.includes('SpriteMaterial'));
check('storm-edge-ribbons-are-meshes', files.runtime.includes('wispCount = mobile ? 6 : 9') && files.runtime.includes('SWVisualSlice6EdgeWisp') && files.runtime.includes('widthToHeight'));
check('storm-directional-ground-pull', files.runtime.includes('SWVisualSlice6GroundPull') && files.runtime.includes('swVisualHeroSlice6GroundShearGeometry') && files.runtime.includes('stormGroundPullCount'));
...
check('qa-asymmetry-probe', files.qa.includes('warpedShellCount') && files.qa.includes('raggedShellCount') && files.qa.includes('lateralCenterOffset'));
...
check('qa-connected-storm-body', files.qa.includes('verticalStreakGeometryCount') && files.qa.includes('condensationStreakCount') && files.qa.includes('groundPullCount') && files.qa.includes('groundPullSpriteCount'));
check('qa-no-inherited-round-storm-sprites', files.qa.includes('visibleInheritedRoundStormSpriteCount') && files.qa.includes('visibleInheritedStormShellCount') && files.qa.includes('visibleInheritedStormRainTraceCount') && files.qa.includes('visibleFoundationStormDustCount') && files.qa.includes('visibleProductionTornadoDodecahedronCount') && files.qa.includes('visibleProductionTornadoBoxDebrisCount') && files.qa.includes('visibleLegacyStormPointCount') && files.qa.includes('legacyDustBowlVisible') && files.qa.includes('Visible inherited round storm sprites remain') && files.qa.includes('Visible inherited storm cone shells remain') && files.qa.includes('Visible inherited square rain-trace sprites remain') && files.qa.includes('Visible round visual-foundation storm dust remains') && files.qa.includes('Visible production tornado dodecahedron fragments remain') && files.qa.includes('Visible production tornado box debris remains') && files.qa.includes('Visible legacy storm point effects remain') && files.qa.includes('Historical dust-bowl presentation remains visible'));
