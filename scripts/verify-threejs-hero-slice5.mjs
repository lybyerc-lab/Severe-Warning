...
const files = {
  ...
  polish: await readFile(path.join(projectRoot, 'runtime', 'threejs-visual-hero-slice5-polish.js'), 'utf8'),
...
try { new vm.Script(files.polish, { filename: 'threejs-visual-hero-slice5-polish.js' }); } catch (error) { polishSyntaxError = String(error?.message || error); }
...
check('slice5-polish-syntax', polishSyntaxError === null, polishSyntaxError || 'ok');
...
check('slice5-polish-marker', files.polish.includes('[SW:VISUAL:HERO_SLICE5:POLISH]') && files.polish.includes('THREEJS_VISUAL_HERO_SLICE5_POLISH_V1'));
...
check('slice5-bridge', files.runtime.includes('__SW_THREEJS_VISUAL_FOUNDATION__') && files.runtime.includes('heroSlice5Version') && files.neonGate.includes('heroSlice5NeonGateVersion') && files.polish.includes('heroSlice5PolishVersion'));
...
check('ordered-after-slice4', files.patch.includes('slice4Index > slice5Index') && files.patch.includes('slice5Index > neonGateIndex') && files.patch.includes('neonGateIndex > guardIndex') && files.patch.includes('guardIndex > polishIndex') && files.patch.includes('polishIndex > loopIndex'));
...
check('patch-injects-polish', files.patch.includes('threejs-visual-hero-slice5-polish.js') && files.patch.includes('[SW:SOURCE:threejs-visual-hero-slice5-polish.js]'));
...
check('rainbow-ribbons-stay-additive', files.runtime.includes('THREE.AdditiveBlending') && files.runtime.includes('SWVisualSlice5RainbowRibbon'));
check('rainbow-polish-shells-normal-blending', files.polish.includes('object.material.blending = THREE.NormalBlending'));
check('rainbow-polish-opacity-budget', files.polish.includes('baseOpacity = index === 0 ? 0.13 : 0.07') && files.polish.includes('object.material.opacity = 0.24 +'));
check('rainbow-polish-glow-budget', files.polish.includes("object.intensity = (mobile ? 0.08 : 0.12)"));
check('rainbow-cycle', files.runtime.includes('material.color.setHSL') && files.polish.includes('object.material.color.setHSL'));
check('legacy-funnel-dimmed-only-by-neon-layer', files.polish.includes('funnelMat.opacity = 0.12') && files.polish.includes('outerFunnelMat.opacity = 0.07') && files.neonGate.includes('if (!selected)'));
...
check('cow-level-instanced-detail', files.polish.includes('new THREE.InstancedMesh') && files.polish.includes('cowMeshBudget: 55'));
check('cow-level-distance-budget', files.polish.includes('cowVisibilityDistance: 165') && files.polish.includes("qaMode === 'cow-level'"));
check('cow-level-dedicated-qa-camera', files.polish.includes("mode !== 'cow-level'") && files.polish.includes('camera.position.set(center.x + 28'));
...
check('qa-proves-balanced-neon', files.qa.includes('shellUsesNormalBlending') && files.qa.includes('maxShellOpacity') && files.qa.includes('glowIntensity'));
check('qa-proves-slice4-restored', files.qa.includes('Hero Slice 4 funnel was not restored after Neon was deselected'));
check('qa-uses-dedicated-cow-view', files.qa.includes("prepareQaView('cow-level')"));
check('qa-enforces-cow-mesh-budget', files.qa.includes('visualMeshCount) <= 55') && files.qa.includes('instancedMeshCount') && files.qa.includes('totalInstances'));
...
check('milestone-records-run3-correction', files.milestone.includes('Run #3 visual review') && files.milestone.includes('55-mesh'));
...
const cosmeticSelectionWrites = [...runtimeText.matchAll(/\bneonFunnelUnlocked\s*(?:\+\+|--|[+\-*/]?=(?!=))/g)].map((match) => match[0]);
...
const report = {
  version: 'THREEJS_HERO_SLICE5_STATIC_V3',
