...
if ((smoke.maxContinuationAttempts ?? 0) !== 0) errors.push('smoke must not use continuation');
...
const slotContinuationBudget = slot.continuationTokenBudget ?? slot.tokenBudget;
if (!Number.isInteger(slotContinuationBudget) || slotContinuationBudget < 2000 || slotContinuationBudget > 50000) errors.push('task slot continuation token budget outside worker limits');
const slotContinuationAttempts = slot.maxContinuationAttempts ?? 0;
if (!Number.isInteger(slotContinuationAttempts) || slotContinuationAttempts < 0 || slotContinuationAttempts > 1) errors.push('task slot continuation attempts must be 0 or 1');
...
requireText(worker, 'maxContinuationAttempts must be 0 or 1', 'worker');
requireText(worker, "input: 'continue'", 'worker');
requireText(worker, 'previous_interaction_id', 'worker');
requireText(worker, 'continuationTokenBudget', 'worker');
requireText(worker, 'interactionChain', 'worker');
requireText(worker, 'continuationCount', 'worker');
requireText(worker, 'Antigravity environment changed across continuation', 'worker');
...
console.log('- incomplete interactions fail closed unless a task explicitly allows one bounded continuation');
console.log('- continuation reuses only the preserved Antigravity environment and literal continue input');
...
console.log('- no bootstrap, hook, V2, or V3 machinery');
