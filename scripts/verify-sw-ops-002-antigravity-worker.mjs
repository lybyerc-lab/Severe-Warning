...
requireText(bootstrapHelper, 'if tool_call.get("name") != "code_execution"', 'bootstrap helper');
requireText(bootstrapHelper, 'respond("deny", "SW-OPS-002 permits code_execution only.")', 'bootstrap helper');
...
console.log('- bootstrap: catch-all pre-tool exact-base airlock, 4000 tokens max, clean-tree host proof before edit turn');
console.log('- command gate: hook rejects non-code tools; exact initial/correction scripts plus tiny read-only Git checks only');
