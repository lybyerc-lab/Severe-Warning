...
const compatiblePhases = new Set(['phase-4-scoring-campaign', 'phase-5-rendering-world']);
...
for (const viewport of viewports) {
  ...
  await page.waitForFunction(() => (
    ...
    && ['phase-4-scoring-campaign', 'phase-5-rendering-world'].includes(
  ...
  const evidence = await page.evaluate(() => {
    ...
    if (!shell || !bridge) throw new Error('Phase 4 shell or bridge unavailable.');
    ...
    return {
      ...
      contractProbe,
      snapshot,
      stopIds: shell.campaign.stops.map((stop) => stop.id),
  ...
  const checks = {
    ...
    playerSaveUntouchedByProbe: evidence.playerSaveUnchanged,
...
await browser.close();
const report = {
  version: 'MODERNIZATION_PHASE4_SCORING_CAMPAIGN_QA_V3',
...
await writeFile(
  path.join(outputDir, 'scoring-campaign-report.json'),
