#!/usr/bin/env node
// [SW:ART:003:MESHY_CLI]
// Candidate asset tooling only. This file must never be imported by shipped runtime code.

import { createHash } from 'node:crypto';
import { mkdir, readFile, writeFile } from 'node:fs/promises';
import path from 'node:path';

const MARKER = 'SW_ART_003_MESHY_CLI_V1';
...
function validateRequest(request) {
  if (request.version !== 'SW_MESHY_LIVE_REQUEST_V1') fail(`Unexpected request version: ${request.version}`);
  ...
  if (request.meshy?.modelType !== 'smart-topology') fail(`Cost guard requires smart-topology: ${request.meshy?.modelType}`);
  if (request.meshy?.aiModel !== 'meshy-t2') fail(`Cost guard requires meshy-t2: ${request.meshy?.aiModel}`);
  if (!Number.isInteger(request.meshy?.targetPolycount) || request.meshy.targetPolycount < 100 || request.meshy.targetPolycount > 15000) {
    fail(`Invalid smart-topology targetPolycount: ${request.meshy?.targetPolycount}`);
  }
  if (request.meshy?.shouldTexture !== true) fail('Cow 17 proof requires one textured candidate.');
  if (request.meshy?.textureResolution !== '2k') fail(`Cost guard requires 2k texture: ${request.meshy?.textureResolution}`);
  if (request.meshy?.enablePbr !== false) fail('First proof keeps PBR disabled for mobile restraint.');
  if (request.meshy?.poseMode !== '') fail('First proof preserves source pose; poseMode must be empty.');
  if (JSON.stringify(request.meshy?.targetFormats) !== JSON.stringify(['glb'])) fail('First proof must request GLB only.');
  if (request.costGuard?.authorizedTasks !== 1) fail('Exactly one Meshy task is authorized.');
  if (!Number.isInteger(request.costGuard?.maxCredits) || request.costGuard.maxCredits > 15) {
    fail(`Credit guard exceeds 15: ${request.costGuard?.maxCredits}`);
  }
}
...
function buildPayload(request, dataUri) {
  return {
    ...
    target_polycount: request.meshy.targetPolycount,
...
async function main() {
  ...
  const dryReport = {
    version: 'SW_ART_003_MESHY_DRY_RUN_V1',
    marker: MARKER,
    dryRun: true,
    request: request.id,
  ...
  await writeFile(path.join(outputDir, 'sw-art-003-task-created.json'), `${JSON.stringify({
    version: 'SW_ART_003_TASK_CREATED_V1',
    request: request.id,
    taskId,
  ...
  const selectedTask = {
    version: 'SW_ART_003_TASK_FINAL_V1',
    request: request.id,
    taskId,
    status: task.status,
  ...
  await writeFile(path.join(outputDir, 'sw-art-003-task-final.json'), `${JSON.stringify(selectedTask, null, 2)}\n`, 'utf8');
  ...
  const receipt = {
    version: 'SW_ART_003_MESHY_RECEIPT_V1',
    marker: MARKER,
    sourceSha: process.env.GITHUB_SHA || null,
    request: request.id,
    taskId,
    ...
    source: {
      ...
      preparedImageSha256: imageHash,
    },
    meshy: {
      modelType: request.meshy.modelType,
      aiModel: request.meshy.aiModel,
      targetPolycount: request.meshy.targetPolycount,
  ...
  await writeFile(path.join(outputDir, 'sw-art-003-receipt.json'), `${JSON.stringify(receipt, null, 2)}\n`, 'utf8');
  console.log(JSON.stringify(receipt, null, 2));
}

main().catch((error) => {
  console.error(`SW-ART-003 Meshy pipeline failed: ${error.message}`);
  process.exit(1);
});
