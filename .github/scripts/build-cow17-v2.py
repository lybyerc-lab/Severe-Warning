...
OUT=os.environ.get('OUT','/mnt/data/cow-17.glb')
...
blob=patch_glb_material(blob); os.makedirs(os.path.dirname(OUT),exist_ok=True); open(OUT,'wb').write(blob)
b=open(OUT,'rb').read(); off=12; jl,jt=struct.unpack_from('<I4s',b,off); off+=8; j=json.loads(b[off:off+jl].decode().rstrip(' \x00')); prim=j['meshes'][0]['primitives'][0]; pos=j['accessors'][prim['attributes']['POSITION']]; idx=j['accessors'][prim['indices']]; tri=idx['count']//3
print('WROTE',OUT); print('bytes',len(b),'triangles',tri); print('dims',[round(pos['max'][i]-pos['min'][i],5) for i in range(3)]); print('minY',pos['min'][1],'meshes',len(j['meshes']),'prims',len(j['meshes'][0]['primitives'])); print('attrs',prim['attributes']); print('material',j['materials'][0]); print('extensions',j.get('extensionsRequired'),j.get('extensionsUsed')); print('images',j.get('images'),'textures',j.get('textures'),'animations',j.get('animations'))
